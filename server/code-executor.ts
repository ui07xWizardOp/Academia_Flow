    ...
    return new Promise((resolve) => {
      ...
      dockerProcess.on('error', (error) => {
        ...
        if (error.message.includes('ENOENT') || error.message.includes('docker')) {
          resolve({
            stdout: '',
            stderr: 'Docker is not available in this environment. For production deployment, ensure Docker is installed and running.',
            exitCode: -1,
            runtime: Date.now() - startTime,
            memory: 0,
            error: 'Docker not available - This feature requires Docker to be installed and running for secure code execution'
          });
        } else {
          resolve({
            stdout: '',
